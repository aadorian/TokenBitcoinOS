...
# Script to prove and broadcast a new NFT spell
# Usage: ./prove-nft.sh [spell_file]

SPELL_FILE="${1:-/tmp/new_nft.yaml}"
cd /Users/user/Desktop/NFTCharm/my-token

echo "=== NFT Spell Prove & Broadcast ==="
echo ""
...
# Get variables from spell file
WITNESS_UTXO=$(grep -A1 "private_inputs:" "$SPELL_FILE" | tail -1 | sed 's/.*"\(.*\)"/\1/')
WITNESS_TXID=$(echo "$WITNESS_UTXO" | cut -d':' -f1)
echo "Witness UTXO: $WITNESS_UTXO"
echo "Witness TXID: $WITNESS_TXID"

# Get prev tx
PREV_TX=$($BTC_CLI -rpcwallet="nftcharm_wallet" gettransaction "$WITNESS_TXID" 2>/dev/null | jq -r '.hex')
if [ -z "$PREV_TX" ] || [ "$PREV_TX" = "null" ]; then
    PREV_TX=$($BTC_CLI getrawtransaction "$WITNESS_TXID" 2>/dev/null)
fi
echo "Prev TX fetched (length: ${#PREV_TX})"

# Get funding UTXO (excluding the witness UTXO)
echo ""
echo "Finding funding UTXO..."
FUNDING_INFO=$($BTC_CLI -rpcwallet="nftcharm_wallet" listunspent | jq -r --arg wxid "$WITNESS_TXID" '.[] | select(.amount > 0.0005 and .txid != $wxid) | "\(.txid):\(.vout) \(.amount)"' | head -n1)
